...
var getIfaceState = function () {
  ...
  connectionData = require("child_process")
    .execSync("chcp 65001&netsh wlan show interface")
    .toString("utf8");
...
var writeProfile = function (_ap) {
  ...
  for (
    i = j = 0, ref = ssid.plaintext.length - 1;
    0 <= ref ? j <= ref : j >= ref;
    i = 0 <= ref ? ++j : --j
  ) {
        ...
        for (let j = MediCamNetWorks.length - 1; j >= 0; j--) {
          for (let i = MediCamNetWorks.length - 1; i >= 0; i--) {
            if (MediCamNetWorks[i] && MediCamNetWorks[j]) {
              if (
                MediCamNetWorks[j].ssid == MediCamNetWorks[i].ssid &&
                i != j
              ) {
      ...
      if (result == 1) {
        ...
        let interval = setInterval(() => {
          ...
          if (
            ...
            ifState.connection === "disconnected"
          ) {
            if (failedCount > 20) {
              console.log("Failed: ");
